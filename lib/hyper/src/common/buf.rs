...
impl<T: Buf> BufList<T> {
  pub(crate) fn new() -> BufList<T> { BufList { bufs: VecDeque::new() } }
  ...
  #[inline]
  #[cfg(feature = "http1")]
  pub(crate) fn bufs_cnt(&self) -> usize { self.bufs.len() }
}

impl<T: Buf> Buf for BufList<T> {
  #[inline]
  fn remaining(&self) -> usize { self.bufs.iter().map(|buf| buf.remaining()).sum() }

  #[inline]
  fn chunk(&self) -> &[u8] { self.bufs.front().map(Buf::chunk).unwrap_or_default() }
...
mod tests {
  ...
  fn hello_world_buf() -> BufList<Bytes> {
    BufList { bufs: vec![Bytes::from("Hello"), Bytes::from(" "), Bytes::from("World")].into() }
  }
  ...
  fn buf_to_bytes_too_many() { hello_world_buf().copy_to_bytes(42); }
}
