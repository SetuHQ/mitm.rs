...
impl Drop for Sender {
  fn drop(&mut self) { self.send(CLOSED); }
}
...
impl Receiver {
  ...
  pub(crate) fn peek(&self) -> Value { self.shared.value.load(Ordering::Relaxed) }
}
