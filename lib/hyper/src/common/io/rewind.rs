...
impl<T> Rewind<T> {
  ...
  pub(crate) fn new(io: T) -> Self { Rewind { pre: None, inner: io } }

  pub(crate) fn new_buffered(io: T, buf: Bytes) -> Self { Rewind { pre: Some(buf), inner: io } }
  ...
  pub(crate) fn into_inner(self) -> (T, Bytes) { (self.inner, self.pre.unwrap_or_else(Bytes::new)) }
