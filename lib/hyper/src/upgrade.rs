...
pub fn on<T: sealed::CanUpgrade>(msg: T) -> OnUpgrade { msg.on_upgrade() }
...
impl Upgraded {
  ...
  where
    ...
    Upgraded { io: Rewind::new_buffered(Box::new(io), read_buf) }
  }
  ...
  pub fn downcast<T: AsyncRead + AsyncWrite + Unpin + 'static>(self) -> Result<Parts<T>, Self> {
    ...
    match io.__hyper_downcast() {
      Ok(t) => Ok(Parts { io: *t, read_buf: buf, _inner: () }),
      Err(io) => Err(Upgraded { io: Rewind::new_buffered(io, buf) }),
    }
  }
}
...
impl AsyncWrite for Upgraded {
  ...
  fn is_write_vectored(&self) -> bool { self.io.is_write_vectored() }
}

impl fmt::Debug for Upgraded {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { f.debug_struct("Upgraded").finish() }
}

// ===== impl OnUpgrade =====

impl OnUpgrade {
  pub(super) fn none() -> Self { OnUpgrade { rx: None } }

  #[cfg(feature = "http1")]
  pub(super) fn is_none(&self) -> bool { self.rx.is_none() }
}
...
impl fmt::Debug for OnUpgrade {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { f.debug_struct("OnUpgrade").finish() }
}
...
impl fmt::Display for UpgradeExpected {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { f.write_str("upgrade expected but not completed") }
}
...
pub(super) trait Io: AsyncRead + AsyncWrite + Unpin + 'static {
  fn __hyper_type_id(&self) -> TypeId { TypeId::of::<Self>() }
}
...
mod sealed {
  ...
  impl CanUpgrade for http::Request<crate::Body> {
    fn on_upgrade(mut self) -> OnUpgrade { self.extensions_mut().remove::<OnUpgrade>().unwrap_or_else(OnUpgrade::none) }
  }

  impl CanUpgrade for &'_ mut http::Request<crate::Body> {
    fn on_upgrade(self) -> OnUpgrade { self.extensions_mut().remove::<OnUpgrade>().unwrap_or_else(OnUpgrade::none) }
  }
...
mod tests {
  ...
  impl AsyncRead for Mock {
    fn poll_read(self: Pin<&mut Self>, _cx: &mut task::Context<'_>, _buf: &mut ReadBuf<'_>) -> Poll<io::Result<()>> {
