...
use crate::common::{task, watch, Future, Pin, Poll};
...
impl Body {
  ...
  /// ```
  #[inline]
  pub fn empty() -> Body { Body::new(Kind::Once(None)) }
  ...
  pub(crate) fn new_channel(content_length: DecodedLength, wanter: bool) -> (Sender, Body) {
    ...
    let tx = Sender { want_rx, data_tx, trailers_tx: Some(trailers_tx) };
    let rx = Body::new(Kind::Chan { content_length, want_tx, data_rx, trailers_rx });
  ...
  fn new(kind: Kind) -> Body { Body { kind, extra: None } }

  #[cfg(all(feature = "http2", any(feature = "client", feature = "server")))]
  pub(crate) fn h2(recv: h2::RecvStream, content_length: DecodedLength, ping: ping::Recorder) -> Self {
    let body = Body::new(Kind::H2 { ping, content_length, recv });
  ...
  fn take_delayed_eof(&mut self) -> Option<DelayEof> { self.extra.as_mut().and_then(|extra| extra.delayed_eof.take()) }

  #[cfg(any(feature = "http1", feature = "http2"))]
  fn extra_mut(&mut self) -> &mut Extra { self.extra.get_or_insert_with(|| Box::new(Extra { delayed_eof: None })) }
  ...
  fn poll_eof(&mut self, cx: &mut task::Context<'_>) -> Poll<Option<crate::Result<Bytes>>> {
    match self.take_delayed_eof() {
      ...
      #[cfg(any(not(any(feature = "http1", feature = "http2")), not(feature = "client")))]
  ...
  fn poll_inner(&mut self, cx: &mut task::Context<'_>) -> Poll<Option<crate::Result<Bytes>>> {
    match self.kind {
      ...
      Kind::Chan { content_length: ref mut len, ref mut data_rx, ref mut want_tx, .. } => {
...
impl From<Box<dyn Stream<Item = Result<Bytes, Box<dyn StdError + Send + Sync>>> + Send>> for Body {
  #[inline]
  fn from(stream: Box<dyn Stream<Item = Result<Bytes, Box<dyn StdError + Send + Sync>>> + Send>) -> Body {
...
impl From<Vec<u8>> for Body {
  #[inline]
  fn from(vec: Vec<u8>) -> Body { Body::from(Bytes::from(vec)) }
}

impl From<&'static [u8]> for Body {
  #[inline]
  fn from(slice: &'static [u8]) -> Body { Body::from(Bytes::from(slice)) }
}
...
impl From<String> for Body {
  #[inline]
  fn from(s: String) -> Body { Body::from(Bytes::from(s.into_bytes())) }
}

impl From<&'static str> for Body {
  #[inline]
  fn from(slice: &'static str) -> Body { Body::from(Bytes::from(slice.as_bytes())) }
}
...
impl Sender {
  ...
  async fn ready(&mut self) -> crate::Result<()> { futures_util::future::poll_fn(|cx| self.poll_ready(cx)).await }
  ...
  pub fn try_send_data(&mut self, chunk: Bytes) -> Result<(), Bytes> {
    self.data_tx.try_send(Ok(chunk)).map_err(|err| err.into_inner().expect("just sent Ok"))
  }
...
mod tests {
  ...
  fn test_size_of() {
    ...
    assert!(body_size <= body_expected_size, "Body size = {} <= {}", body_size, body_expected_size,);
