...
impl Connected {
  ...
  pub fn new() -> Connected { Connected { alpn: Alpn::None, is_proxied: false, extra: None } }
  ...
  pub fn is_proxied(&self) -> bool { self.is_proxied }
  ...
  pub fn is_negotiated_h2(&self) -> bool { self.alpn == Alpn::H2 }

  // Don't public expose that `Connected` is `Clone`, unsure if we want to
  // keep that contract...
  #[cfg(feature = "http2")]
  pub(super) fn clone(&self) -> Connected {
    Connected { alpn: self.alpn.clone(), is_proxied: self.is_proxied, extra: self.extra.clone() }
  }
}
...
impl Extra {
  pub(super) fn set(&self, res: &mut Extensions) { self.0.set(res); }
}

impl Clone for Extra {
  fn clone(&self) -> Extra { Extra(self.0.clone_box()) }
}

impl fmt::Debug for Extra {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { f.debug_struct("Extra").finish() }
}
...
{
  fn clone_box(&self) -> Box<dyn ExtraInner> { Box::new(self.clone()) }

  fn set(&self, res: &mut Extensions) { res.insert(self.0.clone()); }
}

struct ExtraChain<T>(Box<dyn ExtraInner>, T);

impl<T: Clone> Clone for ExtraChain<T> {
  fn clone(&self) -> Self { ExtraChain(self.0.clone_box(), self.1.clone()) }
}
...
pub(super) mod sealed {
    ...
    T: AsyncRead + AsyncWrite + Connection + Unpin + Send + 'static,
  {
    ...
    fn connect(self, _: Internal, dst: Uri) -> crate::service::Oneshot<S, Uri> { crate::service::oneshot(self, dst) }
  }
    ...
    T: AsyncRead + AsyncWrite + Connection + Unpin + Send + 'static,
  {
    ...
    fn connect(self, _: Internal, dst: Uri) -> Self::Future { crate::service::oneshot(self, dst) }
  }
