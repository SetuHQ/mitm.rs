...
impl Config {
  pub(super) fn is_enabled(&self) -> bool { self.max_idle_per_host > 0 }
}
...
impl<T> Pool<T> {
  ...
  fn is_enabled(&self) -> bool { self.inner.is_some() }
...
impl<T: Poolable> Pool<T> {
  ...
  pub(super) fn checkout(&self, key: Key) -> Checkout<T> { Checkout { key, pool: self.clone(), waiter: None } }
...
impl<T> Clone for Pool<T> {
  fn clone(&self) -> Pool<T> { Pool { inner: self.inner.clone() } }
}
...
impl<T: Poolable> Pooled<T> {
  pub(super) fn is_reused(&self) -> bool { self.is_reused }

  pub(super) fn is_pool_enabled(&self) -> bool { self.pool.0.is_some() }

  fn as_ref(&self) -> &T { self.value.as_ref().expect("not dropped") }

  fn as_mut(&mut self) -> &mut T { self.value.as_mut().expect("not dropped") }
}
...
impl<T: Poolable> fmt::Debug for Pooled<T> {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { f.debug_struct("Pooled").field("key", &self.key).finish() }
}
...
impl Expiration {
  fn new(dur: Option<Duration>) -> Expiration { Expiration(dur) }
...
impl<T> WeakOpt<T> {
  fn none() -> Self { WeakOpt(None) }

  fn downgrade(arc: &Arc<T>) -> Self { WeakOpt(Some(Arc::downgrade(arc))) }

  fn upgrade(&self) -> Option<Arc<T>> { self.0.as_ref().and_then(Weak::upgrade) }
}
...
mod tests {
  ...
  use crate::common::exec::Exec;
  use crate::common::{task, Future, Pin};
  ...
  impl<T: Send + 'static + Unpin> Poolable for Uniq<T> {
    ...
    fn reserve(self) -> Reservation<Self> { Reservation::Unique(self) }

    fn can_share(&self) -> bool { false }
  }

  fn c<T: Poolable>(key: Key) -> Connecting<T> { Connecting { key, pool: WeakOpt::none() } }

  fn host_key(s: &str) -> Key { (http::uri::Scheme::HTTP, s.parse().expect("host key")) }

  fn pool_no_timer<T>() -> Pool<T> { pool_max_idle_no_timer(::std::usize::MAX) }
  ...
  async fn test_pool_timer_removes_expired() {
    ...
    let pool = Pool::new(
      super::Config { idle_timeout: Some(Duration::from_millis(10)), max_idle_per_host: std::usize::MAX },
      &Exec::Default,
    );
