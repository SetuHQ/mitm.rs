...
use super::conn;
use super::connect::sealed::Connect;
use super::connect::{self, Alpn, Connected, Connection};
use super::pool::{self, Key as PoolKey, Pool, Poolable, Pooled, Reservation};
#[cfg(feature = "tcp")]
use super::HttpConnector;
use crate::body::{Body, HttpBody};
use crate::common::exec::BoxSendFuture;
use crate::common::{lazy as hyper_lazy, task, Future, Lazy, Pin, Poll};
...
impl Client<HttpConnector, Body> {
  ...
  #[inline]
  pub fn new() -> Client<HttpConnector, Body> { Builder::default().build_http() }
}

#[cfg(feature = "tcp")]
impl Default for Client<HttpConnector, Body> {
  fn default() -> Client<HttpConnector, Body> { Client::new() }
}
...
impl Client<(), Body> {
  ...
  /// use hyper::Client;
  ///
  /// let client = Client::builder().pool_idle_timeout(Duration::from_secs(30)).http2_only(true).build_http();
  ...
  B::Error: Into<Box<dyn StdError + Send + Sync>>,
{
    ...
    match req.version() {
      ...
      Version::HTTP_10 => {
        if is_http_connect {
          ...
          return ResponseFuture::new(Box::pin(future::err(crate::Error::new_user_unsupported_request_method())));
        }
      }
  ...
  async fn retryably_send_request(self, mut req: Request<B>, pool_key: PoolKey) -> crate::Result<Response<Body>> {
  ...
  async fn send_request(&self, mut req: Request<B>, pool_key: PoolKey) -> Result<Response<Body>, ClientError<B>> {
    ...
    let fut = pooled.send_request_retryable(req).map_err(ClientError::map_with_reused(pooled.is_reused()));
  ...
  pub async fn connection_for(&self, pool_key: PoolKey) -> Result<Pooled<PoolClient<B>>, ClientError<B>> {
    ...
    // - Meanwhile, the pool Checkout is watching to see if any other request finishes and tries to insert an idle
    //   connection.
    // - If a new connection is started, but the Checkout wins after (an idle connection became available first), the
    //   started connection future is spawned into the runtime to complete, and then be inserted into the pool as an
    //   idle connection.
  ...
  fn connect_to(&self, pool_key: PoolKey) -> impl Lazy<Output = crate::Result<Pooled<PoolClient<B>>>> + Unpin {
    ...
    hyper_lazy(move || {
      ...
      Either::Left(connector.connect(connect::sealed::Internal, dst).map_err(crate::Error::new_connect).and_then(
        move |io| {
          ...
          Either::Left(Box::pin(async move {
            ...
            executor.execute(conn.map_err(|e| debug!("client connection error: {}", e)).map(|_| ()));
  ...
  B::Error: Into<Box<dyn StdError + Send + Sync>>,
{
  ...
  fn poll_ready(&mut self, _: &mut task::Context<'_>) -> Poll<Result<(), Self::Error>> { Poll::Ready(Ok(())) }

  fn call(&mut self, req: Request<B>) -> Self::Future { self.request(req) }
}
...
impl<C, B> fmt::Debug for Client<C, B> {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { f.debug_struct("Client").finish() }
}

// ===== impl ResponseFuture =====

impl ResponseFuture {
  fn new(fut: Pin<Box<dyn Future<Output = crate::Result<Response<Body>>> + Send>>) -> Self { Self { inner: fut } }

  fn error_version(ver: Version) -> Self {
    warn!("Request has unsupported version \"{:?}\"", ver);
    ResponseFuture::new(Box::pin(future::err(crate::Error::new_user_unsupported_version())))
  }
}

impl fmt::Debug for ResponseFuture {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { f.pad("Future<Response>") }
}
...
impl<B> PoolClient<B> {
  ...
  fn is_http1(&self) -> bool { !self.is_http2() }
  ...
  B: Send + 'static,
{
  ...
  fn reserve(self) -> Reservation<Self> {
    match self.tx {
      PoolTx::Http1(tx) => Reservation::Unique(PoolClient { conn_info: self.conn_info, tx: PoolTx::Http1(tx) }),
      #[cfg(feature = "http2")]
      PoolTx::Http2(tx) => {
  ...
  fn can_share(&self) -> bool { self.is_http2() }
}
  ...
  fn map_with_reused(conn_reused: bool) -> impl Fn((crate::Error, Option<Request<B>>)) -> Self {
    move |(err, orig_req)| {
      if let Some(req) = orig_req {
        ClientError::Canceled { connection_reused: conn_reused, reason: err, req }
      } else {
...
impl Builder {
  #[doc(hidden)]
  #[deprecated(note = "name is confusing, to disable the connection pool, call pool_max_idle_per_host(0)")]
...
mod unit_tests {
  ...
  #[test]
  fn test_is_secure() {
    assert_eq!(is_schema_secure(&"http://hyper.rs".parse::<Uri>().unwrap()), false);
    assert_eq!(is_schema_secure(&"hyper.rs".parse::<Uri>().unwrap()), false);
    assert_eq!(is_schema_secure(&"wss://hyper.rs".parse::<Uri>().unwrap()), true);
    assert_eq!(is_schema_secure(&"ws://hyper.rs".parse::<Uri>().unwrap()), false);
  }
  ...
  fn test_get_non_default_port() {
    ...
    assert_eq!(get_non_default_port(&"http://hyper.rs:123".parse::<Uri>().unwrap()).unwrap().as_u16(), 123);
    assert_eq!(get_non_default_port(&"https://hyper.rs:80".parse::<Uri>().unwrap()).unwrap().as_u16(), 80);
    assert_eq!(get_non_default_port(&"hyper.rs:123".parse::<Uri>().unwrap()).unwrap().as_u16(), 123);
  }
}
