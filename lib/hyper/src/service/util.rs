  ...
  ResBody: HttpBody,
{
  ...
  fn call(&mut self, req: Request<ReqBody>) -> Self::Future { (self.f)(req) }
}

impl<F, R> fmt::Debug for ServiceFn<F, R> {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { f.debug_struct("impl Service").finish() }
}
...
where
  F: Clone,
{
  fn clone(&self) -> Self { ServiceFn { f: self.f.clone(), _req: PhantomData } }
}
